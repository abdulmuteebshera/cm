...
@endphp
<section class="crm-card" id="ec-campaign-live" data-status-url="{{ route('ec.admin.campaigns.status', $campaign->id) }}">
    <div class="crm-card__head">
        ...
        <div class="crm-btn-row">
            @if(in_array($campaign->status, ['draft', 'paused']))
                <form method="post" action="{{ route('ec.admin.campaigns.start', $campaign->id) }}">@csrf<button class="crm-btn crm-btn--accent crm-btn--sm">Start</button></form>
            @endif
            @if($campaign->status === 'running')
                <form method="post" action="{{ route('ec.admin.campaigns.pause', $campaign->id) }}">@csrf<button class="crm-btn crm-btn--ghost crm-btn--sm">Pause</button></form>
            @endif
            @if($campaign->status === 'paused')
                <form method="post" action="{{ route('ec.admin.campaigns.resume', $campaign->id) }}">@csrf<button class="crm-btn crm-btn--accent crm-btn--sm">Resume</button></form>
            @endif
        </div>
    </div>
...
<section class="crm-card" style="margin-top:16px">
    ...
    @else
        <form method="post" action="{{ route('ec.admin.campaigns.load_template', $campaign->id) }}" class="crm-form crm-form__grid" style="margin-bottom:12px">
            @csrf
        ...
        <form method="post" action="{{ route('ec.admin.campaigns.content', $campaign->id) }}" class="crm-form crm-form--stack">
            @csrf
...
<section class="crm-card" style="margin-top:16px">
    ...
    @if($campaign->status !== 'running')
        <form method="post" action="{{ route('ec.admin.campaigns.recipients.add', $campaign->id) }}" class="crm-form crm-form__grid">
            @csrf
        ...
        <form method="post" action="{{ route('ec.admin.campaigns.recipients.import', $campaign->id) }}" enctype="multipart/form-data" class="crm-form crm-form--stack" style="margin-top:12px">
            @csrf
        ...
        @if($campaign->ec_group_id)
            <form method="post" action="{{ route('ec.admin.campaigns.sync_group', $campaign->id) }}" style="margin-top:8px">@csrf<button class="crm-btn crm-btn--ghost crm-btn--sm">Sync from linked group</button></form>
        @endif
    @endif
