...
{
    public function strategyPeriodPayouts()
    {
        try {
            \App\Lib\StrategyPayoutService::syncDuePeriodReturnRecords();
        } catch (\Throwable $th) {
    ...
    public function interest()
    {
        try {
            ...
            $invests = Invest::with('plan.timeSetting', 'user')
                ->where('status', 1)
                ->where('next_time', '<=', $now)
                ->whereHas('plan', fn ($plan) => $plan->where('plan_mode', Plan::MODE_LEGACY))
                ->orderBy('last_time')
                ->take(100)
                ->get();
