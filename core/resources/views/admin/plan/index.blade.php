                            ...
                            <tbody>
                                @forelse($plans as $plan)
                                    <tr>
                                        <td>
                                            @if($plan->isStrategy())
                                                <span class="badge badge--primary">@lang('Strategy')</span>
                                            @else
                                                <span class="badge badge--dark">@lang('Fixed Plan')</span>
                                            @endif
                                        </td>
                                        <td>{{ __($plan->name) }}</td>
                                        <td>
                                        ...
                                        <td>
                                            @if($plan->isStrategy())
                                                <span>@lang('Weekly performance') · {{ $plan->payoutFrequencyLabel() }}</span>
                                            @else
                                        ...
                                        <td>
                                            @if($plan->isStrategy())
                                                {{ $plan->payoutFrequencyLabel() }}
                                            @else
                                                {{ @$plan->timeSetting->time }} @lang('Hours')
                                            @endif
                                        </td>
                                        <td>
                                            @if ($plan->featured == 1)
                                        ...
                                        <td>
                                            @if($plan->isStrategy())
                                                @if($plan->usesMonthlyPerformanceTracking())
                                                    <a href="{{ route('admin.strategy.monthly.returns', $plan->id) }}" class="btn btn-sm btn-outline--info me-2">@lang('Monthly Performance')</a>
                                                @else
                                                    <a href="{{ route('admin.strategy.weekly.returns', $plan->id) }}" class="btn btn-sm btn-outline--info me-2">@lang('Weekly Performance')</a>
                                                @endif
                                                <a href="{{ route('admin.strategy.period.returns', $plan->id) }}" class="btn btn-sm btn-outline--warning me-2">@lang('Period Returns')</a>
                                            @endif
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="plan_mode" class="form-control plan-mode-select" required>
                                        <option value="0">@lang('Fixed HYIP Plan')</option>
                                        <option value="1">@lang('Strategy (Weekly performance + period payout)')</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6 strategy-only-field" style="display:none">
                                <div class="form-group">
                                    <label>@lang('Payout Frequency')</label>
                                    <select name="payout_frequency" class="form-control">
                                        <option value="quarterly">@lang('Quarterly')</option>
                                        <option value="semi_annual">@lang('6 Months')</option>
    ...
    <script>
        (function($) {
            ...
            $('.modalShow').on('click', function() {
                ...
                modal.find('[name=plan_mode]').change(function() {
                    plan.togglePlanMode($(this).val());
                }).change();
            });
            ...
            class HyipPlan {
                constructor(modal, btn) {
                    ...
                    if (this.resource) {
                        ...
                        this.modal.find('[name=plan_mode]').val(this.resource.plan_mode ?? 0);
                        if (this.resource.payout_frequency) {
                            this.modal.find('[name=payout_frequency]').val(this.resource.payout_frequency);
                        }
                    ...
                    this.togglePlanMode(this.modal.find('[name=plan_mode]').val());
                }

                togglePlanMode(mode) {
                    var isStrategy = String(mode) === '1';
                    this.modal.find('.strategy-only-field').toggle(isStrategy);
                    this.modal.find('.legacy-plan-fields').toggle(!isStrategy);
                    this.modal.find('.legacy-plan-fields').find('input, select').prop('disabled', isStrategy);
                    this.modal.find('[name=interest]').prop('required', !isStrategy);
                    this.modal.find('[name=time]').prop('required', !isStrategy);
                    this.modal.find('[name=return_type]').prop('required', !isStrategy);
                }
